<template>
	<div class="tabs-wrapper p-4">
		...
		>
			<div class="tab-item">
				<GeneratorsSummaryTabContent>
					<GeneratorsSummaryTabUrl :user="user" :credits="creditsStore.credits" />
				</GeneratorsSummaryTabContent>
			</div>
		</div>
		<div v-if="$creditLoader === 'not-loaded' || $creditLoader === 'loading'" class="mt-4 flex justify-center">
			<Loader />
		</div>
...
import { summaryTabsStore } from '~/store/summary-generator.store'
const user = useSupabaseUser()
const client = useSupabaseClient()
const store = summaryTabsStore
...
const getUserCredits = async () => {
	...
	const { data, error } = await creditsStore.getCredits(client, user?.value.id)
	if (error) {
